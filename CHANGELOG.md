...
- [application-manager, bundle-plugin, cli] removed the webpack bundler; `theia build` now always bundles with esbuild, and the new `compressAssetsPlugin` of `@theia/bundle-plugin` emits the pre-compressed `.gz` frontend assets that webpack's `CompressionPlugin` used to produce [#17935](https://github.com/eclipse-theia/theia/pull/17935)
